...
// Fetch code written with Teammate 1
function fetchData() {
    const url = "http://127.0.0.1:8028/api/users/";
    const options = {
        method: 'GET',
...
<!--  -->
    ...
    const images = []
            for (let j = 0; j < lenIngredients; j++){
                ...
                images.push('/lmc-frontend/images/' + ownedItems[j] + '.png')
            }
    console.log(images)
...
// Define the bake function
    // Get references to the ingredient containers
    function bake() {
        ...
        // Array to store the IDs of ingredients
